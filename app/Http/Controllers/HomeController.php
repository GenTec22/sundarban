...
class HomeController extends Controller
{
    ...
    $monthlySales = Sales::selectRaw('SUM(amount) as total_amount, MONTH(created_at) as month')
    ...
    $topSales = Sales::select('product_id', DB::raw('SUM(amount) as total_sales'))
        ->groupBy('product_id')
    ...
    $todaySales = Sales::whereDate('created_at', $today)->sum('amount');
    $yesterdaySales = Sales::whereDate('created_at', $yesterday)->sum('amount');

    // Fetch this week's sales
    $thisWeekSales = Sales::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
                        ->sum('amount');

    // Fetch last week's sales
    $lastWeekSales = Sales::whereBetween('created_at', [Carbon::now()->subWeek()->startOfWeek(), Carbon::now()->subWeek()->endOfWeek()])
                        ->sum('amount');
